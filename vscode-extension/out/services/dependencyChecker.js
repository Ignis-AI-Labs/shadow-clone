...
class DependencyChecker {
    ...
    getTooltipText(status) {
        ...
        if (!status.claudeInstalled)
            missing.push('Claude Code CLI');
        return `Missing: ${missing.join(', ')}\nClick to install`;
    }
    async installDependencies(context) {
        const status = await this.checkAllDependencies();
        // Show setup guide
        const panel = vscode.window.createWebviewPanel('shadowCloneDependencies', 'Shadow Clone Setup', vscode.ViewColumn.One, {
            enableScripts: true,
    ...
    getSetupWebviewContent(status, context) {
    ...
    async installNodeJs() {
        const choice = await vscode.window.showInformationMessage('To install Node.js, you need to download it from nodejs.org or use a package manager.', 'Open nodejs.org', 'Show Commands');
    ...
    async installClaudeCode() {
        ...
        vscode.window.showInformationMessage('Installing Claude Code in the terminal. After installation, you may need to restart VS Code.', 'OK');
    }
    async runSetupScript(context) {
        ...
        vscode.window.showInformationMessage('Running setup script. Please wait for it to complete, then restart VS Code.', 'OK');
    }
    dispose() {
