...
class SecurityTelemetryService {
    ...
    async handleSuspiciousActivity(check) {
        ...
        if (this.suspicionScore >= 50) {
            vscode.window.showWarningMessage('Unusual activity detected. This session is being monitored for security.', 'Acknowledge');
        }
     ...
     */
    async handleHighRiskActivity() {
        // Notify the backend
        try {
            const apiKey = await this.authProvider.getApiKey();
            if (!apiKey)
                return;
            await fetch(`${(0, constants_1.getApiEndpoint)()}/api/security/high-risk-alert`, {
                method: 'POST',
                headers: {
        ...
        vscode.window.showErrorMessage('Security violation detected. Some features may be restricted.', 'Contact Support').then(selection => {
