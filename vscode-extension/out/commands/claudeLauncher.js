...
async function launchClaudeCommand(sessionManager, authProvider) {
    ...
    try {
        ...
        const action = await vscode.window.showInformationMessage(`Shadow Clone command ready. Review before sending?`, { modal: true, detail: finalCommand }, 'Send to Claude', 'Edit First', 'Cancel');
        if (action === 'Send to Claude') {
            // Send the command directly to the terminal
            claudeTerminal.sendText(finalCommand);
            vscode.window.showInformationMessage('Shadow Clone command sent! Claude will fetch the prompt from the API.', 'Track Session').then(choice => {
            ...
            if (editedCommand) {
                ...
                vscode.window.showInformationMessage('Custom Shadow Clone command sent!', 'Track Session').then(choice => {
                    if (choice === 'Track Session') {
                        vscode.commands.executeCommand('shadowClone.showSessions');
                    }
                });
