...
1. **Question** (header `Question`) — the specific research question. Free-text.
2. **Deliverable format** (header `Format`) — options: `Markdown report`, `Decision matrix`, `Code sketch / spike`, `Slide-ready summary`.
3. **Citation depth** (header `Citations`) — options: `Primary sources required`, `Reputable secondary OK`, `Anything credible`.
4. **Time budget** (header `Budget`) — options: `Quick (30-60 min)`, `Standard (a few hours)`, `Deep (full day+)`.
