...
1. **Symptom** (header `Symptom`) — one paragraph describing what's wrong. Free-text.
2. **Reproduction** (header `Repro`) — exact steps to trigger the bug, or `unknown` if not yet found. Free-text.
3. **Suspected area** (header `Suspect`) — options: `Frontend`, `Backend`, `Data layer`, `Integration / network`, `Build / deploy`, `Unknown`.
4. **Reverting acceptable?** (header `Revert`) — options: `Yes — revert is fine`, `No — must forward-fix`, `Depends on RCA`.
