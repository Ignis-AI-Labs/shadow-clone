...
rewiring imports), that is `/sc-refactor`, not Cleaner — recommend it and stop.
...
## Step 8 — Dispatch `/sc-echo` paired review (if active)
...
When a `/sc-echo` review flags a violation, it cites the protocol filename so
you can verify.
