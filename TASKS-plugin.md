...
| P-P1-07 | Grok reviewer backend for `/sc-echo` — `bridge/ask-grok.sh` (read-only via grok `--deny`, fail-closed MCP pre-flight, argv/CWE-200 warning, truncation-safe per-pass budget + persona debit), backend selection via `SC_REVIEWER_BACKEND` / `/sc-echo grok`, `SC_MIN_PASS_AVAIL` chunker floor, sc-doctor probe | DONE | @eli | P-P1-02 | echo-approved (claude backend, no findings); GROK-001/002 filed+fixed |
