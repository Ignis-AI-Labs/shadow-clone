---
name: sc
description: "Activate Shadow Clone in this project — discover the codebase, adapt protocols into local AGENTS.md / CLAUDE.md, set up branching, and orient the user to the /skill:sc and /skill:sc-* skill surface"
type: prompt
whenToUse: When the user asks to initialize, activate, or set up Shadow Clone in their project (run once per project, or refresh an existing setup)
---
...
- A clear understanding of which `/skill:sc-*` mode to invoke next.
...
- **All three exist** → tell the user "Shadow Clone is already initialized in this project." Offer two options: `Refresh` (re-derive AGENTS.md from current protocols and re-confirm answers) or `Skip` (orient them to `/skill:sc-help` instead). Wait for their choice.
...
3. **Team size** (header `Team`) — options: `Solo`, `2-3`, `4-7`, `8+`. Drives the per-wave spawn cap for every `/skill:sc-*` mode in this project.
...
<mode_overview>
  <purpose>
    ...
    branching and audit scaffolding, and leave the user knowing which `/skill:sc-*`
    mode to invoke next. This is the only mode that intentionally writes files
    outside `.waves/` — it bootstraps the project's law.
  </purpose>

  <why_important>
    Every other `/skill:sc-*` mode assumes `AGENTS.md` exists at the repo root and
    accurately reflects the project's standards. Without `/skill:sc`, that assumption
    is unmet — modes either fall back to the bundled template (a generic
    starter) or fail their precondition. `/skill:sc` makes the project ready to use
  ...
  <wave_2>
    ...
    <deliverables>
      <deliverable path=".waves/wave-2/deliverables/SETUP_COMPLETE.md">
        ...
        - Suggested next move: which `/skill:sc-*` mode to invoke for the user's actual work
    ...
    <instructions>
      ...
      4. The final SETUP_COMPLETE.md is the user's handoff document — make it actionable: explicit "next: run `/skill:sc-plan` to plan your first feature" or similar based on Type/Stack/Stakes.
    </instructions>
  </wave_2>
</wave_structure>
...
<init_guidelines>
  ...
  <principle>
    ...
    points at the next move — which `/skill:sc-*` mode, with what scope, in what order.
  </principle>
</init_guidelines>
...
<success_criteria>
  ...
  <criterion>SETUP_COMPLETE.md orients the user to which /skill:sc-* mode to invoke next</criterion>
...
After each wave's deliverable is written, briefly report to the user: what was produced, where it landed, what the next wave will do. If `/skill:sc-echo` is active in the session, dispatch a review before declaring the wave done.
