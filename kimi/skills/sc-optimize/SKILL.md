...
type: prompt
whenToUse: When the user asks to optimize performance, reduce resource usage or cost, or wants measure-first performance improvement with baselines and targets
---
...
Wait for the answers. If `Baseline` is `Need to measure first`, Wave 0 starts with instrumentation. Echo a one-line scope confirmation, then proceed to Step 1.5 (run initialization).
