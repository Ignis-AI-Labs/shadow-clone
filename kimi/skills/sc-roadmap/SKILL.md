...
type: prompt
whenToUse: When the user wants to plan a multi-milestone roadmap — sequencing workstreams with dependencies, parallel tracks, and decision gates across an initiative larger than one sprint
---
...
It is **not** the right mode for greenfield project planning (use `/skill:sc-plan`) or for a single upcoming sprint (use `/skill:sc-sprint`).
