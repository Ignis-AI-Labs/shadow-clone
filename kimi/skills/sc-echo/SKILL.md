---
name: sc-echo
description: "Enter echo paired-review mode — a second model reviews each completed work unit against AGENTS.md and returns a VERDICT line (APPROVE / REVISE / BLOCK / ERROR)"
type: prompt
whenToUse: When the user asks to enable Shadow Clone echo mode, paired review, or wants each work unit reviewed by a second model
arguments:
  - backend
---
...
Echo routes each review through one reviewer CLI. Resolve which is active for this
session **once, now**, in this order:

1. If the user passed an argument to this skill (`$backend`), use it:
   - `claude` → the **Claude** reviewer via `ask-claude.sh`.
   - `opencode` (or `glm`) → the **GLM via OpenCode** reviewer via `ask-glm.sh`.
   - `grok` → the **Grok** reviewer via `ask-grok.sh`.
   - `kimi` → the **Kimi** reviewer via `ask-kimi.sh` (note: self-review — a
     separate `kimi -p` process reviews your work).
2. Otherwise use the configured default: read `SC_REVIEWER_BACKEND` from
   `~/.config/sc/config`. If unset, use `claude` (the default when Kimi is the
   Builder). A configured value of `opencode`, `grok`, or `kimi` is also valid.
...
backend, tell them the valid choices (`claude`, `opencode`, `grok`, `kimi`) and
ask which they want before continuing.
...
```
bash ~/.claude/sc/ask-claude.sh "<context>" <path1> <path2> ...   # claude backend (default)
