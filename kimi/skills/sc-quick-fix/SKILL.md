...
## Step 5 — Dispatch `/skill:sc-echo` paired review (if active)

If echo mode is on (the user has activated `/skill:sc-echo` in this session),
...
5. **Anything you noticed but didn't change** — if you saw a related
   issue out of scope, mention it as a `/skill:sc-quick-fix` follow-up or a
   `/skill:sc-feature`/`/skill:sc-refactor` candidate.
