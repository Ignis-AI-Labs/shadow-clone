...
Use this when:
- A feature just shipped (often after `/skill:sc-feature` or `/skill:sc-quick-fix`)
  and needs test coverage.
- An existing module has coverage gaps you want filled deliberately.
- You've run `/skill:sc-test-audit` and want to act on a specific finding.
...
- If a contract is too tangled to test cleanly, **say so** in the
  ...
  `/skill:sc-refactor` to untangle it first.
...
- **Next steps** — if you noticed code that needs `/skill:sc-refactor` or
  `/skill:sc-debug`, list it. Tests reveal design issues; surface them.

If `/skill:sc-echo` is active in the session, dispatch a review of the new
