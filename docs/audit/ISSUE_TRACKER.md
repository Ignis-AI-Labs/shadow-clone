...
- **Fixed By**: Builder (Claude)
- **Date Fixed**: 2026-06-30 (partial)
- **Fix Description (partial)**: `workspace-initializer.ts` split — 462 → 299 lines (under 300 ceiling). The four `getXxxContent()` template-string methods extracted into a new `workspace-templates.ts` (165 lines, under 200 target) as exported `const` strings (they were pure — no `this`, no IO — so the method shape was incidental). One bash split: `sc_echo_review`'s verdict-aggregation pass extracted into `_sc_aggregate_verdict` in `chunk-review.sh` (sc_echo_review 114 → 102 lines).
- **Deferred (acknowledged)**: (a) The remaining three over-ceiling bash functions in `bridge/lib/` (`sc_echo_review` still 102, `sc_build_request` 112, `sc_invoke_reviewer` 102) are tangled with shared state (`FILES`, `CONTEXT`, `REQ`, `RESP`, etc.) that bash can only pass via globals; further extraction risks readability without removing the global coupling. Scheduled as a separate `refactor/bridge-globals` task that introduces an explicit state struct (associative array). (b) The ~37 oversized TS handlers in `prompts/content/` and the `modular/*.ts` handlers (76–129 lines each) are each a single cohesive prompt-builder; splitting them yields helper functions whose only call site is the parent, which trades the size violation for a worse cohesion-vs-locality tradeoff. House standard preserves them as-is; the 200-line *file* ceiling (the stricter constraint) is satisfied. (c) QA-013 TASKS-*.md "Edit on dev" wording — one-line edit per file deferred to the next docs-polish PR alongside QA-015 / QA-016.

- **Issue ID**: AUDIT-027
- **Status**: RESOLVED 2026-06-30 (P3 polish pass)
- **Discovered By**: Reviewer (Application Security, Wave 1)
- **Date Discovered**: 2026-06-30
...
- **Fixed By**: Builder (Claude)
- **Date Fixed**: 2026-06-30
- **Fix Description**: AS-006 → `sc_assert_env_ident` in `bridge/lib/guard.sh` rejects `SC_REVIEWER_MODEL` / `SC_REVIEWER_AGENT` / `SC_CLAUDE_MODEL` values containing anything outside `[A-Za-z0-9._/-]`; both `bridge/ask-glm.sh` and `bridge/ask-claude.sh` call it before the readonly assignments so a `~/.config/sc/config` author cannot append `--dangerous-flag` to the reviewer CLI. AS-009 → `sc_assert_project_dir` refuses `PROJECT_DIR=` (empty), `/`, or bare `${HOME}` — the file-containment filter would otherwise let every absolute path through. AS-007 + AS-012 → unit-level smoke tests added at `bridge/tests/guard.bats` covering both new guards plus `_sc_uint_or`; full `--disallowedTools` / Bun `$` integration tests deferred (require running CLIs). All three bridge scripts re-parse with `bash -n`; the guard library compiles cleanly.

- **Issue ID**: AUDIT-028
- **Status**: PARTIALLY RESOLVED 2026-06-30 (P3 polish pass; QA-015 / QA-016 deferred)
- **Discovered By**: Reviewer (multiple Wave 1 specialists)
- **Date Discovered**: 2026-06-30
...
- **Fixed By**: Builder (Claude)
- **Date Fixed**: 2026-06-30
- **Fix Description (partial)**: SC-010 already closed in Theme 3 README data-egress section. SC-012 → `CONTRIBUTING.md` now has a "Lockfile policy" section explaining `package-lock.json` is authoritative and `npm ci` is the install command. SC-013 → `web/package.json` declares `"engines": { "node": ">=20" }`. SC-014 → new `opencode-plugin/package.json` declares the package name, `type: module`, `engines.node`, and the `@opencode-ai/plugin` peer dep. QA-014 → `CONTRIBUTING.md` size-limits section now leads with a "House overrides" callout pointing at the canonical protocol (400/500) the project's stricter 200/300 ceilings sit on top of. **Deferred**: QA-015 (cite Testing & QA Protocol from `sc-feature` / `sc-test-audit` Standards blocks) and QA-016 (note `sc-echo` / `sc-help` divergence from 3-wave shape in-file) are command-content edits in `claude/commands/` — scheduled for the next slash-command-content PR rather than this docs-polish pass.
...
- **Fixed By**: Builder (Claude)
- **Date Fixed**: 2026-06-30
- **Fix Description**: `docs/audit/POSITIVE_FINDINGS.md` now captures each verified-correct property (AS-013 re-entrancy guard, AS-007 argv shape, AS-012 Bun template safety, IS-008 cksum tradeoff, QA-012 impurity annotations, the AUDIT-020 module-size win, the Hono cluster reachability assessment) along with "don't regress" guidance so future audits / refactors preserve the contract. `npm audit fix` in `mcp-server/` bumped `hono@4.12.23 → 4.12.27` (one better than the 4.12.25 target); `npm audit` now reports 0 vulnerabilities.
...
- **Status**: RESOLVED 2026-06-30 (P3 polish pass — promoted from Deferred)
...
- **Fixed By**: Builder (Claude)
- **Date Fixed**: 2026-06-30
- **R3 echo deferrals (Info, accepted)**: (a) The string-valued getters now use `if (value === undefined)` (matches `??` semantics exactly), the record-valued ones keep `if (!value)` — semantically equivalent for `Record<string, NonNullObject>` lookups but kept simpler. (b) Caller chain: every modular handler passes a `z.infer<typeof XSchema>` enum value, so the throw path is unreachable in normal traffic; if a future caller bypasses the schema, the throw propagates to `combinedTools.executeTool` → MCP `InternalError`. No `getXOrDefault` variant added — the fail-fast posture is intentional. (c) The bats fallback assertions use a loose substring match `[[ "$output" == *"300" ]]`; acceptable for a smoke test, exact-match could be added later if a stream-splitting helper is introduced.
- **Fix Description**: All 9 lookup helpers across the 8 config files now throw `Error("Unknown <param>: ${key}")` on lookup miss instead of returning a default. Breakdown: 5 record-value getters (`getTeamTemplates`, `getSpecialistInfo`, `getWaveConfiguration`, `getDocumentationConfig`, `getConsultationConfig`), 2 string-value getters (`getReviewFocus`, `getIssueGuidance`), and 2 helpers in `testing-guidelines.ts` (`getTestingGuidelines` returns string, `getDefaultFramework` returns string). The schema-enforced enums in `src/schemas/tool-schemas.ts` keep miss-paths unreachable in normal traffic, but the throw means a future drift between schema and lookup table fails loud at runtime instead of silently returning the wrong prompt. `tsc --noEmit` passes.
...
- **Description**: The clause "Every wave deliverable … and rk-operations file this mode produces lands under `<run-dir>/wave-N/...`" uses the term `rk-operations` without defining it inline. The term IS defined in the canonical Wave & Subagent Coordination Protocol §2.5 (dir tree + audit-trail section), and the clause is byte-identical across the wave modes that carry it — so it is protocol-backed and consistent, but a receiving AI that hasn't read the protocol cannot act on the term from the command file alone.
- **Fix (shipped)**: Added a self-contained parenthetical — "rk-operations file (the Record Keeper's run-coordination audit trail, defined in the Wave & Subagent Coordination Protocol §2.5)" — to every mode that carries the Step 1.5 clause: `sc-tests`, `sc-docs` (echo-approved, finalization review round 2) plus `sc-feature`, `sc-debug`, `sc-refactor`, `sc-research`, `sc-audit`, `sc-optimize` (identical approved string, propagated mechanically; verified by word-diff that the sole change per file is the added parenthetical — closed mechanical observation, no collateral change).
- **Scope note**: `sc-plan`, `sc-sprint`, `sc-roadmap` do NOT use the Step 1.5 sentence form; they already self-document `rk-operations/` inline via their wave dir-tree comments (`# ONLY: AGENT_ASSIGNMENTS.md, RECORD_KEEPER_LOG.md, WAVE_COMPLETE.md`), so no change was needed. All 11 wave modes now define the term at point of use.
